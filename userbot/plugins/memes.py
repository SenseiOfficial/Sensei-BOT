...
async def repo(e):
    if not e.text[0].isalpha() and e.text[0] not in ("/", "#", "@", "!"):
        await e.edit(
            "🔱⇋Itz 🔥G.O.A.T.🔥  [⚜️─ѕєиѕєιвσт─⚜️](https://github.com/SenseiOfficial/Sensei-BOT) here babes⇋🔱 !"
        )
...
async def source(sed):
    if not sed.text[0].isalpha() and e.text[0] not in ("/", "#", "@", "!"):
        await sed.edit(
            "⚜️─Wanna know who am I ? If yes , just click 👉 [Here](https://github.com/SenseiOfficial/Sensei-BOT)─⚜️ !"
        )
